defmodule NVim.Plugin do
  ...
  defmacro __before_compile__(_env) do
    quote do
      ...
      def nvim_specs, do: Map.values(@specs)
    end
  end

  def eval_specs(params), do: Enum.filter(params, fn {k, _} -> k in [:eval] end)
  def autocmd_specs(params), do: Enum.filter(params, fn {k, _} -> k in [:group, :nested] end)
  def command_other_specs(params), do: Enum.filter(params, fn {k, _} -> k in [:buffer, :bar] end)

  def command_values_specs(params),
    do: Enum.filter(params, fn {k, _} -> k in [:range, :count, :bang, :register] end)

  def command_args_specs(params) do
    Enum.filter(params, fn {k, _} -> k in [:range, :count, :bang, :register, :eval] end)
  end

  def wrap_reply({status, reply, state}, _, _) when status in [:ok, :error],
    do: {:reply, {status, reply}, state}

  def wrap_reply(other, initstate, _), do: {:reply, {:ok, other}, initstate}
  ...
  def do_deffunc(signature, funcparams, body) do
    ...
    eval = "[#{eval_specs |> Keyword.values() |> Enum.join(",")}]"
    ...
    spec =
      quote do
        %{
          ...
          opts: %{unquote_splicing(opts)}
        }
      end

    quote do
      @specs if(@specs[unquote(name)],
               do: @specs,
               else: Map.put(@specs, unquote(name), unquote(spec))
             )
      def handle_call(
            {:function, unquote(name), unquote(call_args)},
            _nvim_from,
            unquote(state) = initialstate
          )
  ...
  defmacro deffunc(signature, funcparams \\ [], do: body) do
    do_deffunc(signature, funcparams, body)
  end

  def do_defcommand(signature, funcparams, body) do
    {name, params, guard} = sig_to_sigwhen(signature)
    ...
    special_dict = Enum.zip(Keyword.keys(values_specs), special_args)
    nvim_arg_order = [range: 0, count: 1, bang: 2, register: 3]

    special_args =
      special_dict
      |> Enum.sort_by(fn {k, _} -> nvim_arg_order[k] end)
      |> Keyword.values()

    with_defaults = Enum.reverse(params)

    without_defaults =
      Enum.map(with_defaults, fn
        {:\\, _, [e, _]} -> e
        e -> e
      end)
    ...
    quote do
                   ...
                   opts: %{
                     unquote_splicing(
                       if(nargs_args == [], do: [], else: [nargs: "*"]) ++
                         Enum.map(values_specs ++ command_other_specs(funcparams), fn {k, v} ->
                           {k, wrap_spec_value(k, v)}
                         end) ++
                         if(eval_specs == [],
                           do: [],
                           else: [eval: "[#{eval_specs |> Keyword.values() |> Enum.join(",")}]"]
                         )
                     )
                   }
                 })
      ...
      def handle_call(
            {:command, unquote(name), unquote(call_args)},
            _nvim_from,
            unquote(state) = initialstate
          )
          when unquote(guard) do
      ...
      if unquote(nargs_args !== []) do
        def handle_call(
              {:command, unquote(name), unquote(default_call_args) = [nargs | other_args]},
              from,
              unquote(state) = initialstate
            )
            when unquote(guard) do
  ...
  defmacro defcommand(signature, funcparams \\ [], do: body) do
    do_defcommand(signature, funcparams, body)
  end
  ...
  defmacro defautocmd(signature, funcparams \\ [], do: body) do
    ...
    quote do
               ...
               do: @specs,
                 ...
                 Map.put(@specs, unquote(name), %{
                   ...
                   opts: %{
                     unquote_splicing(
                       [pattern: pattern] ++
                         Enum.map(autocmd_specs(funcparams), fn {k, v} ->
                           {k, wrap_spec_value(k, v)}
                         end) ++
                         if(eval_specs == [],
                           do: [],
      ...
      def handle_call(
            {:autocmd, {unquote(name), unquote(pattern)}, unquote(call_args)},
            _nvim_from,
            unquote(state) = initialstate
          )
          when unquote(guard) do
