defmodule NVim.App do
  ...
  defmodule Sup do
    ...
    def gen_api do
      if Application.get_env(:neovim, :update_api_on_startup, true), do: NVim.Api.from_instance()
      :ignore
    end
  end
end
...
defmodule IOLeaderSink do
  def handle(from, reply_as, {:put_chars, _, _}), do: send(from, {:io_reply, reply_as, :ok})
  def handle(from, reply_as, {:put_chars, _, _, _, _}), do: send(from, {:io_reply, reply_as, :ok})
  def handle(from, reply_as, {:get_until, _, _, _, _, _}), do: send(from, {:io_reply, reply_as, {:done, :eof, []}})
  def handle(from, reply_as, {:get_chars, _, _, _}), do: send(from, {:io_reply, reply_as, :eof})
  def handle(from, reply_as, {:get_line, _, _}), do: send(from, {:io_reply, reply_as, :eof})
  def handle(from, reply_as, {:setopts, _}), do: send(from, {:io_reply, reply_as, :ok})
  def handle(from, reply_as, :getopts), do: send(from, {:io_reply, reply_as, []})
  def handle(from, reply_as, {:requests, requests}), do: for(r <- requests, do: handle(from, reply_as, r))
  def handle(from, reply_as, _), do: send(from, {:io_reply, reply_as, {:error, :request}})
